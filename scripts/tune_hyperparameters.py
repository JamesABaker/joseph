"""
Hyperparameter tuning for GAN-based AI text detector.

This script performs grid search over key hyperparameters:
- hidden_dim: Network capacity
- learning rates: Generator and discriminator
- latent_dim: Generator noise dimension
- epochs: Training duration

Aggressive search space to squeeze out maximum accuracy.
"""
...
def main():
    ...
    # Aggressive hyperparameter search space
    param_grid = {
        "hidden_dim": [128, 256, 384, 512],  # Network capacity
        "latent_dim": [64, 100, 128],  # Generator noise dimension
        "lr_generator": [0.0001, 0.0002, 0.0003],  # Generator learning rate
        "lr_discriminator": [0.00005, 0.0001, 0.00015],  # Discriminator learning rate
        "epochs": [150, 200, 250],  # Training duration
    }
